...
public:
    ...
    */
    ...
    // Custom function added by me!
    float applyEnvelopeToSample(const float& sample)
    {
        if (state == State::idle)
        {
            return sample;
        ...
        if (state == State::sustain)
        {
            return sample * parameters.sustain;
        }
        auto envelope = getNextSample();
